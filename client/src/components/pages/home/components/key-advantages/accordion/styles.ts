...
export const getStyles = ({ isOpen }: IStylesProps) => ({
  ...
  title: 'font-scada text-[20px] font-bold text-lynch tablet:text-[24px] laptop:text-[26px]',
  arrowWrapper: 'bg-arrowBg h-[36px] min-w-[36px] rounded-full',
  arrow: cn(
  ...
  description: cn(
    'font-scada text-white tablet:text-[20px] laptop:text-[24px] opacity-0 transition-all duration-300',
    isOpen && 'opacity-100',
  ),
});
