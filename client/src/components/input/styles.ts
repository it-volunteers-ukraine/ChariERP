...
export const getStyles = ({
  ...
  fieldset: cn(
    'relative flex align-center pb-3 w-full overflow-hidden transition-all duration-300 border rounded-sm group/item',
    {
      'h-[64px]': type !== 'search',
  ...
  div: 'flex cursor-pointer [&>svg]:peer-focus:text-input-focus',
  ...
  iconCopyDiv: cn('flex cursor-pointer', {
    '[&>svg]:peer-focus:text-input-focus': value,
  }),
  iconCopy: cn({
    'text-light-blue hover:text-dark-blue active:text-green-active active:transition-none': value,
    'cursor-not-allowed hover:none active:none text-black': !value,
  }),
  search: 'w-6 text-light-blue cursor-pointer hover:scale-110 transition-all duration-200 mr-3',
});
